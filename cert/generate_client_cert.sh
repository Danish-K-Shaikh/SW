...
openssl req -new -x509 -days $VALID_DAYS -key $CA_KEY -config openssl.conf -out $CA_CERT
echo "Done."
...
openssl req -new -key $SERVER_KEY -config openssl.conf -out $SERVER_CSR
...
openssl req -new -key $CLIENT_KEY -config openssl.conf -out $CLIENT_CSR
